...
export function ProductSchema({ product }: { product: Product }) {
  const jsonLd = {
    ...
    sku: product.sku,
    image: product.images[0] || "https://hitechairconditioning.in/og-image.jpg",
    brand: {
      "@type": "Brand",
    ...
    offers: {
      "@type": "Offer",
      url: `https://hitechairconditioning.in/product/${product.slug}`,
      priceCurrency: "INR",
...
export function BlogPostSchema({
  ...
  const jsonLd = {
    ...
    headline: title,
    description,
    url: `https://hitechairconditioning.in/${slug}`,
    image: image || "https://hitechairconditioning.in/og-image.jpg",
    datePublished: datePublished || "2026-01-01",
    dateModified: dateModified || datePublished || "2026-01-01",
    author: {
      "@type": "Person",
      name: "Hitech Technical Team",
      jobTitle: "HVAC Engineers",
      url: "https://hitechairconditioning.in/about",
    },
    publisher: {
      "@type": "Organization",
      name: "Hitech Aircool Engineers",
      logo: {
        "@type": "ImageObject",
        url: "https://hitechairconditioning.in/logo.png",
      },
    },
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": `https://hitechairconditioning.in/${slug}`,
    },
...
export function ProductListSchema({
  ...
  const jsonLd = {
    ...
    itemListElement: products.slice(0, 50).map((p, i) => ({
      "@type": "ListItem",
      position: i + 1,
      url: `https://hitechairconditioning.in/product/${p.slug}`,
      name: p.name,
    })),
  };
  return (
